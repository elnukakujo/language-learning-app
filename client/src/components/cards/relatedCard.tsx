...
export default function RelatedCard({ languageId, words = [], characters = [], passages = [] }: RelatedCardProps) {
  ...
  return (
    <section className="rounded-md border p-4 shadow-sm">
      <h2 className="mb-3 text-lg font-semibold">Related Components</h2>
      {groups.length > 0 ? (
                ...
                {group.items.map((item, index) => {
                  ...
                  return (
                    <Link
                      ...
                      href={href}
                      className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-sm text-slate-800 transition hover:bg-slate-100"
                    >
                      {label}
                    </Link>
                  );
        ...
        <p className="text-sm text-slate-500">No related components found.</p>
      )}
    </section>
  );
}
